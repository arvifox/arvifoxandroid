package com.arvifox.hibinyadv.utils.sb;
...
public final class ConvertUtils {
    ...
    @SuppressLint("NewApi")
    public static <E> boolean removeIf(@NonNull List<E> items, @NonNull Predicate<E> filter) {
        Objects.requireNonNull(items);
        Objects.requireNonNull(filter);
        boolean removed = false;
        final Iterator<E> each = items.iterator();
        while (each.hasNext()) {
    ...
    public static String[] toStringArray(@Nullable CharSequence[] items) {
        if (items == null) {
            return NpeUtils.EMPTY_STRING_ARRAY;
        }
    ...
    public static int fromDpToPx(@NonNull final Context context, final float dp) {
